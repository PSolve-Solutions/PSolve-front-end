...
export class StandingComponent implements OnInit {
  ...
  private categorizeTrainees(trainees: Achiver[], totalProblems: number): void {
    const calculatePercentage = (solvedProblems: number) =>
      (solvedProblems / totalProblems) * 100;
    this.greenZone = trainees.filter(
      (t) => calculatePercentage(t.solvedProblems) >= 80
    );
    this.yellowZone = trainees.filter(
      (t) =>
        calculatePercentage(t.solvedProblems) < 80 &&
        calculatePercentage(t.solvedProblems) > 50
    );
    this.redZone = trainees.filter(
      (t) => calculatePercentage(t.solvedProblems) <= 50
    );
    [this.greenZone, this.yellowZone, this.redZone].forEach((zone) =>
      this.sortBySolvedProblems(zone)
    );
  }
