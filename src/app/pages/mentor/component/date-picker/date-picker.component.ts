...
@Component({
  ...
  template: `
    <div class="relative">
      ...
      <div #calendar class="absolute left-0 mt-2 p-4 bg-white border border-gray-200 rounded-md shadow-lg hidden z-10">
        <div class="flex justify-between items-center mb-2">
          <button (click)="changeMonth(-1)" class="focus:outline-none">&lt;</button>
        ...
        <div class="grid grid-cols-7 text-center text-sm mt-2">
          <ng-container *ngFor="let day of days">
            <div class="p-1 cursor-pointer rounded hover:bg-gray-200" (click)="selectDate(day)">
              {{ day }}
            </div>
          </ng-container>
        </div>
...
export class DatePickerComponent {
  ...
  selectDate(day: number) {
    ...
    this.datepickerInput.nativeElement.value = selectedDate.toLocaleDateString('en-US');
  ...
  renderCalendar(date: Date) {
    ...
    this.days = [];
    this.monthYear = date.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
