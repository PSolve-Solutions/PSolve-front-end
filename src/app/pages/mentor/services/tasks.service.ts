...
export class TasksService {
  ...
  getData(id: any): Observable<ResponseHeader> {
    return this.http.get<ResponseHeader>(
      `${environment.BASE_URL}/api/Mentor/tasksOnStatus/${id}`
    );
  }
  getAssign(id: any): Observable<ResponseHeader> {
    return this.http.get<ResponseHeader>(
      `${environment.BASE_URL}/api/Mentor/getTraineeForAssign/${id}`
    );
  }
  trainees(id: any): Observable<ResponseHeader> {
    return this.http.get<ResponseHeader>(
      `${environment.BASE_URL}/api/Mentor/trainees?campId=${id}`
    );
  }
  addTask(data: any): Observable<any> {
    return this.http.post<any>(
      `${environment.BASE_URL}/api/Mentor/tasks`,
      data
    );
  }
